...
```{r}
design <- model.matrix(~ Treatment + Passage, data=sample_table)
...
p + aes(color=Passage, shape=Treatment)
```
...
```{r}
good_design <- model.matrix(~ Treatment + Passage, data=good_sample_table)
...
###Model
```{r}
animal.results <- topTable(fit, coef=c("PassageP5", "PassageP6"), n=Inf)
table(animal.results$adj.P.Val <= 0.1)
```
###New Model
```{r}
animal.good_results <- topTable(fit, coef=c("PassageP5", "PassageP6"), n=Inf)
